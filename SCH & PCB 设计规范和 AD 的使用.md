...
- SCH 设计 -> SCH 查错 -> PCB 设计 -> PCB 查错。
  - SCH 设计的注意：原理图绘制完毕后紧接着添加 线组 Net Class、添加 差分线组、对线组、差分线组或其它如电源线等单独添加规则，这些都尽量在 SCH 中完成（尽量不要在 PCB 界面中另设置，PCB 界面中只专注与画 PCB），方法详看 “AD 20 快捷键 & 绘制技巧” 一节的 “SCH 中的” 部分。针对线宽的规则可以做依不同 线组（Net Class）设置不同线宽规则，并在 Rules 界面的左下角设置优先级（手动布线时低优先级不起作用）；针对高速线的规则要依 线组（Net Class）单独设置规则，详见下面的 "等长线/蛇形线设计" 小节；针对差分线要设置 差分对（Diff Pair）规则，详见下面的 “差分走线设计” 小节。
- 在 PCB 文件中导入 AD20 的 PCB Rules 规则配置文件，参考 "7 AD 导入导出配置文件" 章节。
- SCH & PCB 的检查：
  - 设计时遵守的规范：按照下面 “4 SCH-PCB 设计规范” 小节中介绍的内容绘制和完善 SCH 绘制 和 PCB 布局布线。
  - 设计完之后的查错表格：首要确保 SCH & PCB 全部编译无错，接着检查每个元件的封装、封装 BOM 名称、线路连接、功率线加粗、PCB 上放置元件留出焊接调试空间、地平面覆铜和加泪滴；然后按照检查表格（Check List）检查一遍，参考 “SCH & PCB 检查表 \ Check List” 一节。
- 
- 
...
- 步骤详看“AD 20 快捷键 & 绘制技巧” 小节里面的 “SCH 中” 部分的 “添加差分线对” 部分！
- 下面是计算 PCB 阻抗和 线对的间距 的方法。
