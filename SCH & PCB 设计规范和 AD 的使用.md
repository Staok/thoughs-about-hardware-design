...
[硬件技术进步最快方式：没有之一（邪修版）](https://mp.weixin.qq.com/s/UzvYApjaU7SXx7mJee0MnQ)。
...
- **电阻**：先考虑类别（插件电阻、贴片电阻、水泥电阻等等），阻值，功率，精度（E24-5%，E96-1%），温漂，品牌厂家，封装，价格，热噪声。

  精密电路优先考虑金属膜电阻（精度高，噪声小，稳定）。

  [薄膜电阻与厚膜电阻有何差异？本文一次给你讲透！ | DigiKey](https://www.digikey.cn/zh/blog/cn-what-is-the-difference-between-thin-film-resistance-and-thick-film-resistance)，薄膜电阻可以具有更高的精度（0.1%），薄膜电阻比厚膜电阻有更低的电流噪声。
...
TL431 输出并接滤波电容要至少 10uF 以上，过小会输出震荡。https://www.bilibili.com/video/BV1Rzw5e8EiM。

这个输出滤波电容容量与输出稳定性的关系在 TL431 手册里面有一张图。

其它稳压器件、LDO估计也有类似特性，即 0.1uF 或者更小电容会起到反作用，一般输出电容加大都是安全的。看手册！
...
- mos 驱动线路，为消除高频噪声，可以加上 rc 环节，比如 1k+100pf，或者 磁珠+100pf。
- H桥驱动：
  - h桥，在比如正转旋转的时候，不应该是：正转两个mos管导通和4个mo子管都不导通或者只高边断开和低边仍道通这样切换，应该是：正转的mos管导通和低边两个mos管导通这样切换。
  - 前者会有h桥让电机两个引脚完全悬空的状态，电机的反电动势会无处可去，因此应该采用后者的方式。
  - 另外如果pwm的频率很低比如1KHz，低速转动会比较卡顿或者不连续，应该保持pwm的较高频率。
...
主要是针对 **电源线** 的 抗冲击设计 和 对 **信号线** 的 抗干扰保护（EMS） 与 防电磁辐射（EMI）（合起来为 EMC）。

- 过压，欠压，防反接，软起动（防止打火花。以及针对输入端大电容的情况）——主要针对 电源入口设计。

- 过流，过温——主要针对芯片、发热元件，电路板整体散热规划 等。

- EMC 电磁兼容性设计，如 瞬态抑制 / 防浪涌 TVS，防静电 ESD，以及 共模、差模干扰噪声抑制，以及针对开关震荡和尖峰的 RC 吸收电路，感性负载的 续流二极管等。这些分别用在不同的地方。

  - ESD：手指静电（几千伏）碰到电路板，ESD二极管瞬间导通，把电压泄放掉，保护芯片不被击穿。ESD 二极管是专门为了对付静电（电压极高、但能量极小、持续时间纳秒级）设计的。用于保护 USB 接口 等 这种信号线。

    对于开关断开、线圈/导线反冲”产生的能量（焦耳级），对于 ESD 二极管来说往往太大了，ESD 二极管会因为承受不住感性负载释放的能量而瞬间短路或炸裂。这里必须用功率型的 TVS。注意各有各的使用场景。

  - TVS：电源线上突然来了一个 50V 的尖峰（比如电机急停产生的感应电动势），TVS 导通，把电压死死钳位在 30V，保护后端电路。用来 消尖，但不能消除震荡。

  - RC 抑制尖峰噪声/消火花（“RC吸收电路”（Snubber），通常用于消除开关断开时的尖峰或震荡。场景：当电路中的开关（如机械开关、MOS管或继电器）断开瞬间，或者电流剧烈变化时，导线上的电感（寄生电感）会产生反向电动势，导致电压上下震荡（Ringing），这个震荡会形成高频噪声，对外发出电磁干扰）。RC 用来消除震荡和降低电磁干扰（EMI），降低 dV/dt；利用电容改变寄生震荡的频率，利用电阻消耗震荡的能量。它把原本尖锐、剧烈的震荡波形，变成平缓的波形。

    对于电源入口的消火花这个 RC 方法 并不适用，应该用软起动。详见下面 保护电路总结，以及后面 `电源规范设计` 章节。

  - 对于感性负载（如电机、继电器线圈）：最好的选择 是反向并联一个肖特基二极管（续流二极管），这是这个场景标准的“消火花/消反动势”方案。
...
晶振PCB布线的经验规则 https://www.bilibili.com/video/BV1HQmSYMEvL。
...
1. 一些注意点：
   ...
   - 原理图增加器件，重新对器件编号，只对新增器件编号，没有动的器件的编号不用动，有时候会写文档标注里面的一些器件的编号，因此没有修改的器件的编号就保持不动。
...
1. 先定义PCB规则。点开“设计”->“规则”，主要设置线宽范围，元件最小间距，焊盘的开窗范围，默认过孔尺寸，覆铜类型为直接连接等。
   ...
   2. PCB 打板 最基本 / 最普遍的普通（便宜的） 工艺，[以嘉立创为例](https://www.jlc.com/newOrder/#/pcb/pcbPlaceOrder)，[下单前技术员必看 (jlc.com)](https://www.jlc.com/portal/server_guide_112.html)：
...
2. 确定板尺寸，划定板形。在 Keep-out Layer 层绘制板边界闭合线，然后执行“设计”->“板子形状”->“按照选择对象定义”。
   ...
   如果有镂空，则选择 Define Borad Cutout 来画，按住 ctrl 然拖动边可以新增一个中间点。
...
6. 多层板（多于 2 层的偶数层）。
   ...
   - **多层板中，设计一组或多组临近的电源层和地层，该耦合层之间可形成电容，对噪声可形成泄放**。**注意电源平面比地平面内缩 符合 20H 原则**。

     电源与地平面必须相邻（形成电源和地之间的电容，降低对噪声泄放的阻抗）。

   - **元器件层下面的第二层应为一个完整的地平面，作为地参考**。

     避免地平面被信号线割裂。

     敏感电路区域单独划分地岛，单点接地，并注意防止共地干扰（敏感电路的地应直接进入公共地，而不是还经过其它电路模块的地再到公共地）。

   - 一些推荐分层 [PCB常用分层设计介绍，4层/6层/8层 (qq.com)](https://mp.weixin.qq.com/s/o7qQmuDivcdgsIhr1dwRaA)。

   

   - 四层板举例：性价比很高，这个板层以上价格飙升。
...
2. 布线。在 布局 阶段进行粗布放，布线的时候再进行精细调整，为 器件之间保持合理间距、更方便布线等。
   ...
   - 主要分为电源线、信号线（包括关键信号、敏感信号、模拟信号等）以及底线。
     ...
     - 信号。参考 参考 `4 SCH-PCB 设计规范` 一章里面的 布局布线规范（`幼稚园级`、`本科生级` 小节） 等。
       ...
       - **信号线可以细一点（如 5、6mil），信号线间距符合 3W 原则**，减少辐射和串扰。
   ...
   - **检查布线完成情况**：工具栏 "Report"，"Board information"，勾上 "Routing information"，点 "Report"，新页面中查看布线的完成情况。如果还差最后几根不容易发现的线，在 PCB 页面中以 shitf + s 显示单独层来查看未连接的线。
     ...
     > 接下来选择“<All Nets>”
     >
     > 在下面看到“Un-Routed” 那一列 不为0的就是未连接
   
     [【AD20】如何检查是否有漏线、飞线没有连接_ad20怎么检查线有没有连接-CSDN博客](https://blog.csdn.net/iiinoname/article/details/108106744)。
...
1. 后续工作和润色。
   ...
   - **加泪滴**（选择 线形，非弧形）。如果先覆铜了，那么可以直接加泪滴然后再重铺覆铜即可。teardrop style 选择 line。对于过孔加泪滴，可以设置 泪滴的长宽，按需调整。
     ...
     加泪滴的设置，可以先选择 Force，出现 间距冲突的，如果少量，则手动删除出现冲突的线（则这个线的泪滴会消失）然后重新画上即可修好，如果冲突比较多，就先 加泪滴的设置里面选择 Remove 所有 泪滴，然后再加泪滴 并 不选择 Force（AD20 可能有 bug 这时候还会有冲突的线，则选一个冲突的线删掉，这时候其它所有与泪滴冲突的线的泪滴都会消失，然后再补上刚删的线，就好了）。

     加泪滴后重新覆铜。

   - 3D视角 **检查器件是否重合**，最后检查走线，调整走线（距离适中，减少串扰等等）。

   - 插件是否符合 SMT 的安全间距需求，至少 和贴片元件的间距 5mm。

   - **加丝印**，在丝印层加板子的接口（电源、通讯接口等）、Logo 等标识，注意丝印不要被过孔、开窗、器件等覆盖住。
...
4. 拼板。
   ...
   3. **后处理 / 拼版**。
      ...
      - 对于需要 SMT 的板子，需要加两边的工艺边。具体可以参考如下。

        - 一些工艺辅助的制作规范：

          拼板操作流程：https://www.sz-jlc.com/portal/server_guide_71.html

          V割拼板制作规范：https://www.sz-jlc.com/portal/server_guide_69.html

          邮票孔拼板制作规范：https://www.jlc.com/portal/server_guide_10181.html

        - 拼板时板与板之间的距离一般为1.6mm或2mm的间距。拼板尺寸应考虑板厚及元器件重量等因素（如过薄的板不宜拼太大，生产或运输中易出现板翘），常规板拼板后的长宽尽量控制在150*200MM以内。

        - 三个必须：

          - 工艺边。工艺边的常规宽度为5mm（最小3MM）。工艺边是为了满足自动化贴片的需要而增加的辅助部分，生产完成之后会被移除，一般设计为 5mm。一般 SMT 时候会用到 工艺边 用于 固定板子和定位，这个 如果自己不做，一般 SMT 厂家会帮助去做。

            当PCB板边元件距板边小于3mm时，也应增加工艺边以便于机器组装元器件。当PCB外形或拼版后外形不规则（比如弧形等）时，应在板两侧增加工艺边。

            对于波峰焊工艺，要求元件距离板边大于5mm。

            对于板边特殊的连接器件（如DB插座），工艺边宽度可适当增加。

          - 定位孔。工艺边一般加4个定位孔（一般为大小2MM的无铜孔）。 用于 PCB 加工和测试时的定位，一般设计为 2mm 的过孔。

          - Mark 点 / 光学定位点。在板两面添加2至4个光学定位点（大小1MM，开窗2MM）。用于自动化贴片机进行识别定位，通常设计为 1mm 的焊盘。

      - 拼版后需要分板，一般两种，用 V-Cut 或者 邮票孔，前者适用于切割的地方横平竖直且没有镂空，如果有镂空且板边形状不规则则建议拼板后用邮票孔，注意不要让邮票孔太难掰！

        能V割拼板，尽量改为V割拼，毕竟邮票孔会产生齿状毛刺；采用V割拼板的长宽最小尺寸70*70mm，最大尺寸为长475mm，以符合V割机所允许的加工尺寸。

        采用V割拼板的，V割中心线距离导线的边线或铜皮（焊盘边）的距离≥0.4mm；采用邮票孔拼板的，邮票孔边距离导线的边线或铜皮（焊盘边）的距离≥0.2mm；安装孔或槽距V割中心线或邮票孔需要保证安全距离（最好1MM以上），避免掰板时破孔。

      - 对于邮票孔放置：两板的连接处中间加两排或者一排过孔（邮票孔：一般0.8mm大小的非金属化过孔，间距1.1mm，五个为宜），拿到后可直接掰开，掰开后再处理平滑一下边缘，而省去切割；应该注意便于板掰开的过孔要足够多，不用吝啬，如果寥寥几个过孔的话掰开仍然很困难。

        比如如下用于 SMT 的 且不规则板边的拼版例子（只是左上角一部分）。

        <img src="assets/拼版例子-用于SMT的且不规则板边（只是左上角一部分）.png" alt="拼版例子-用于SMT的且不规则板边（只是左上角一部分）" style="zoom:50%;" />

   4. **后检查**。尤其是，原 PCB 中有一些 不焊接的 N.C 器件，拼版之后这些信息消失了，，检查拼板后不焊接的器件，目前还不知道有什么办法在拼版的 pcb 上 标识 不焊接的器件，目前在 SMT 的时候手动再设置这些器件不焊接。

   5. 输出 pdf 和 BOM：

      1. 用 Export BOM 生成 BOM excel 表格。

      2. 用 Smart PDF，给 原理图 SCH 生成一份 pdf。

      3. 用 Smart PDF，给 拼板 PCB 生成一份 各个器件的 Designator 的位置图，示例如下：

         可以 PCB 里面，全选所有 器件 Components，显示 Designator，切换到 Top Overlay，然后全选 Designator 调整 高度（比如 0.2mm） 和 线宽（比如 0.05mm），并设置居中，一些器件的 Designator 可能被 器件封装的死因遮挡，则只选择这一类器件，可以一块拖动，对于 Bottom Overlay 同理。

         然后在 Smart PDF 的 Printouts & Layers，只保留 Top Overlay 和 Keep-Out Layer，右边勾选 Surface-mount 和 Through-hole，不勾选 Mirror，然后即可导出 正面 的 PCB 各个器件的 Designator 的位置图，对于 Bottom Overlay 同理操作，但是勾选 Mirror。

         <img src="assets/PCB器件Designator位置生成示意.png" alt="PCB器件Designator位置生成示意" style="zoom: 67%;" />
...
- 添加差分对（Diff Pair）：
  ...
  Place 中选择 Directives 里面的 Differential Pair 进行放置。如果放到 Blanket 的上面，则可以 Differential Pair 里面 继续添加 Rule 的 Differential Pairs Routing 来给这个 Pair 线设置差分走线的 线宽、线距等。
...
- 一些接插件、端子等等，选择防反接、防呆设计。

  如果接插件如排针这种没有防反接设计，那么就在接口线序上做设计，就算插反也不会烧毁器件等。

- 具体器件类型相关的使用经验和规则，见 前面 **基本器件使用规则、经验** 小节。

- 更多参考，见下面 **敏感信号线/关键信号线 和 高频信号线/高速信号线 设计的几个原则** 小节。
...
- 模块划分，单点接地。不同频率的器件，一般高速放在接口处，关于地平面，考虑将两者的地分割，然后在接口处单点连接。
  ...
  来去信号，尽量并排走的，怎么去就怎么回，去和回线路中间的面积不要大，避免成一个口子，即避免形成环形。一般，回就是地线，地线走覆铜，所以信号线对应的地线覆铜，应该完整覆铜在信号线的至少上面或者下面，全程"护着"。

  对于敏感信号、震荡信号同理，对于电源线同理，只要走电流的线均应该注意，去和回的线避免成环。
...
- 走线避免出现直角和锐角。T 型 线 改成 Y 型 线。

- 3W 相邻线间距，或相邻线中间加入接地线隔离。

- 减少线并行的长度减小串扰。

- 关键信号线四周铺满地线隔离。关键敏感信号的上下要至少有一个完整地平面。关键敏感信号远离板边缘。
...
- 关键数字信号线 如 MCU 的 IRQ 中断引脚等重要 IO 注意静电保护等，可加防浪涌的 TVS 瞬态抑制管保护，或防正负过压 / 冲击的 ESD 管保护。
  ...
  按键，一端接地，连接 IO，按键并联上一个 双向ESD。

  其实，关键的 IO 线，比如 芯片 的 对外各种 IO 和 通讯接口、usb、mipi、pcie 等等 中的每个线都可以并上 ESD，具体看 一些 EMC 推荐电路。

  差分对线，驱动器件后面 可以串入 共模滤波器（小封装、贴片，参数比如 90Ω@100MHz，抑制高频共模噪声）。
...
- 尽量靠近 mcu 等。
- 确保地线和覆铜围绕晶振，完整包裹。
- （无源）晶振出来的差分信号线并走进入 mcu。其信号线上的接地电容可以放在 进入 mcu 方向的另一边。
...
- **！！！高电压 或 重负载 的电源输入 的 各个 环节 按顺序 汇总（每个环节均可选，看情况加）！！！**：是保护比较全面的工业级电源入口设计。

  - 物理接口。使用带锁扣或高摩擦力的接口（如 XT30/XT60 或 Amass MR30），避免接触不良导致的打火或断电。

  - 保险丝（熔断型（反应快，分断能力强） 或 自恢复型（适合比如＜3A等小电流场景，大电流下内阻大））。对于 TVS 击穿短路（TVS 的常见失效模式）或者后级 Buck 烧毁短路 情况必须需要。

  - TVS。必须在 Fuse 之后，防反接/软启动之前。吸收静电（ESD）和插拔瞬间的高压尖峰。

  - 防反接。低侧用 NMOS，高侧用 PMOS。

  - MOS 软起动。必须在大电容之前。利用 RC 控制 P 或 N 型 MOS 的 Vgs 使其缓慢开启。

    前面二者可以合二为一：防反接 MOS 和 软启动 MOS 可以是同一个管子！通过合理的电路设计（控制 Gate 极的充电速度），可以让防反接管子在上电时兼具缓启动功能。

    有时候这个 MOS 需要 受 IO 控制 用于 开关机 等，需要再设计。

  - EMC（CLC型滤波，L 为共模电感）。在软启动之后，大电容之前（或者包含在大电容环节中）。前面的 C 不用太大。净化电源。

  - 大容量铝电解电容（亚mF 或 mF 级别）。可以是上面 EMC 滤波环节后面的 C。重负载低频滤波、储能平滑电源。

  - MLCC。(0.1uF + 10uF 或 22uF)：必须紧靠 后级电路 芯片电源输入引脚。滤除高频纹波。

  - 泄放电路（断电安全，安规）。在电容两端并联一个 2kΩ~10kΩ 或 以上 的电阻（根据电压和功耗计算），确保断电几秒内电压降到安全值。

  - 后级电路（BUCK 等）。

    还可参考上面 `保护机制` 章节，以及里面的 保护电路合集 的链接，多参考。
...
- 参考 各种 EMC 标准电路，[protection-circuits/EMC 理论和设计要点 at master · Staok/protection-circuits (github.com)](https://github.com/Staok/protection-circuits/tree/master/EMC 理论和设计要点) / [EMC 理论和设计要点 · 瞰百/protection-circuits - 码云 - 开源中国 (gitee.com)](https://gitee.com/staok/protection-circuits/tree/master/EMC 理论和设计要点)，其它保护电路也参考这个仓库里面的电路。
  ...
  并可参考 后面 `EMC 电磁兼容设计` 章节。

- DC 输入接口消火花电路，尤其对于大的容性、感性负载（比如接口处有 亚mF级别 的大的电容），尽量接口处加 软起动 电路；如果是重负载，后面再加大容量的铝电解电容或者固态电容，注意耐压留出一倍裕量。
...
- [电源芯片设计要注意这些！不然纹波很大_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Jh4y1u7tz/)。
...
包括 ADC、运放、RC、PCB、运放和电阻噪声 等等的计算。
...
- 【精】首先精读电路和数模电课本，基本功， 《电路(第五版).邱关源》、《模拟电子技术基础（第四版）童诗白》、《数字电子技术基础（第五版） 阎石》。
...
- 【精】《华为硬件工程师手册》159页，用于补充。

- 【精】[《硬件系统工程师宝典》](https://book.douban.com/subject/26318343/)——EDA精品智汇馆，介绍了硬件系统设计概要、信号完整性、电源完整性、EMC/EMI、原理图和PCB设计详情等，用于补充。

  有一个关于这本书的系列文章，在其专栏。

  [硬件系统工程师宝典（1）-----硬件系统设计应该从哪里开始？_硬件工程师设计宝典-CSDN博客](https://blog.csdn.net/diudiulala/article/details/128396029)。
