...
- DCDC 到 LDO 之间加 LC 环节，LC 衰减高频噪声的频带要覆盖 LDO 的 RSRR 不力的频带。
- 运放供电前加 LC 环节，同样的，LC 衰减高频噪声的频带覆盖运放的 PSRR 不力的频带。
...
- 电阻热噪声：不同电阻的电压和电流噪声如下。经验值，一般的 100M 电阻在 100Hz 频率约有 0.1pA 电流噪声（随着阻值减小），约有 10uV 电压噪声（随着阻值增大）。不同材质、封装 等 的 电阻有不同的噪声表现，金属膜、绕线电阻 的 相对于 一般碳膜 的 较低，具体可见厂家电阻手册。并且考虑 电阻精度。
  ...
  可以看出，如对于减小电压噪声，可采取：减小电阻阻值 或 降低系统带宽。由于硬件系统功耗、电流噪声等的约束，降低电阻阻值不应过低，但可适度通过 RC 等环节 约束 运放电路 的信号带宽，来降低噪声水平。

  关于电阻电流热噪声，可参考 TI 文档 `CMOS 和 JFET 放大器中电流噪声的影响`。

- 运放的电压和电流噪声密度指标，这些噪声经过具体的运放电路连接形式放大到输出端。

  - TIA 电路考虑电流噪声密度（即输入端的）应该远小于被测电流（电流噪声 至少小于 需求被测电流精度 两个数量级，至少 小于 偏置电流 Ib 一个数量级）（还有 Ib 和 Ib 的温漂都应该考虑）。

  - TIA 电路考虑运放电压噪声对输出的贡献，为跨阻和信号源内阻组成的电压放大电路的倍数（也即噪声增益） * 运放电压噪声密度值  * 带宽的开平方。

    对于弱电流信号源，可以认为信号源内阻是很大的，与TIA合适量程的跨阻阻值相当或更大。

  - 运放的增益带宽积（电压型运放的闭环增益和带宽的积通常为一个定值）GBW 不必很大，否则噪声带宽就越大，按需。

- 信号源的噪声。

- 以上还有其它种种，合起来可估算总噪声。具体的噪声计算和估计，可参考下面 `相关书籍文献` 一节里面的，并用 仿真软件（下面 关于 `TIA 反馈网络` 一节里面介绍的） 上手试试。

- 关于噪声、电阻噪声、运放电路噪声计算的一些网文、书籍：

  - [功率谱和频谱的区别-CSDN博客](https://blog.csdn.net/godloveyuxu/article/details/77030793)。[FFT求频谱图和功率谱密度图_幅值谱密度-CSDN博客](https://blog.csdn.net/liubing8609/article/details/85526616)。
  ...
  - [如何抑制电阻器中的噪声干扰 - EMC/EMI设计 - 电子发烧友网 (elecfans.com)](https://www.elecfans.com/emc_emi/1098415.html)。
  - [运算放大器噪声计算详解-CSDN博客](https://blog.csdn.net/TaylorLi421/article/details/105127846)。
  - [【运算放大器】噪声指标（4）实例计算与仿真_运放噪声仿真-CSDN博客](https://blog.csdn.net/weixin_44358304/article/details/140695994)。
  - [信号链类——放大器噪声计算 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/464206112)。
  - （推荐）书籍《运算放大器噪声优化手册》。
  - 长文《Operational Amplifier Noise：Techniques and Tips for Analyzing and Reducing Noise》。
  - 书籍《Low-Noise Electronic System Design》。
  - 书籍《低噪声电子设计-莫特钦巴切尔, 菲特钦》。
...
数模电路分离，单点接地，防共地干扰。电源树设计，多路电源分别与之供电。PCB 两个 高性能的 LDO 分别给 模拟（TIA 和 ADC 部分） 和 数字部分（如 MCU 部分） 供电，电源端，数字部分 和 模拟部分 在单点接地（不要串起来），做好防共地串扰，空间分离。参考 `额外文档\关于接地-解决共地干扰` 文件夹里面的文档！
...
电流输入端过压保护。高输入电压需要考虑，有的 TIA 仪器说输入端最高可 1500V，是运放的 IN- 前面 串了一个 1M 电阻（下面 `相关书籍文献` 一节中的 `图解DIY 1pA超微电流测试器 lymex` 文档中是这么做的）做兼容高压输入。运放输入电阻很大，大电压过来，经过1M电阻 到 运放引脚，就可承受了，也是 `防止因偶然输入过压而导致运放损坏`。具体还得计算和仿真下！
...
- （推荐）系统参考书籍 《跨阻放大器设计参考》，比较全的介绍了围绕 TIA 电路的分析、计算 等的 种种。

- 还要系统参考下 吉时利的 最新《低电平测量手册》 等。

- （推荐）芯片大厂文档，如 ADI、TI 等的。经典如：
  - 跨阻放大器电路设计和计算。
  - ADA4530、LMP7721 等芯片相关的所有技术、应用文档。

- （推荐）CSDN 下载区里面搜索 弱电流测量 相关的文档（有些是比较珍贵的、生动真实的一手的实践经验总结，要的就是这种真实性的）。经典如：
...
- 多看一些论文，关键字有：弱信号采集、弱电流测量，nA测量，pA测量，等，多看多思考多总结。
  ...
  但是搜了一些看下来，有一些中文区论文（尤其是学生写的）相比于上面 大厂文档、专业书籍 和 网友出于兴趣自己做实验的总结文章 相比 不太真实，有水分，不管做的实际如何，但是写的还是忽略了很多细节，与我期待的 对这个领域比较完善的总结、以及非常可信的论证+实验+结果分析的过程 有一定距离（因为真到自己需要做的时候就真的对别人的产出非常有要求了haha），所以务实着来。况且，经费充足、电路板宽大舒展的硬件制作（对于TIA）、比较稳定的实验室环境也能 比较容易 测试出 好的数据，哪怕多测一些就可以有好的数据，但是离工程化、实用化、消费级化还差的比较远，有管理、技术路线、开发工具、测试条件等等的因素。让自己真正学会、真正做到给后人铺路，淌河搭桥（而不是给别人添堵、浪费别人时间），增加有效信息流通效率，整个社会的效率会越来越高。（扯远了）
...
- 微弱信号检测方法及仪器. 戴逸松
- 电子电路抗干扰技术
- 电子系统中的噪声抑制与衰减技术. HenryW.Ott 著. 王培清 译
- 弱信号检测技术. 刘国福
- 电子噪声与低噪声设计. 高晋占
- 弱信号检测. 高晋占
...
- 视具体使用的 ADC 类型，设计滤波器。注意 ADC 的 输入紧接着必须要有 RC 环节，运放输出直接给 ADC 会引入额外噪声。

  - SD ADC，差分输入，ADC 输入做 RC 抗混叠，两个输入做RC，这个C 为 差模滤波电容，后面再接一个共模滤波电容，后者 电容 要比前者 至少大 一个数量级（10倍），滤除采集频率以上的噪声信号。
  - 对于 SAR ADC，则单输入，运放跟随 + 一阶 RC。

- 双T陷波滤波器等（[《信号与系统》4.10.2工频干扰的滤除_抑制工频干扰需要什么滤波器-CSDN博客](https://blog.csdn.net/qq_32589131/article/details/105614591)）（但是占地方，且增加器件增加噪声，不值当，应当：降低系统噪声 + 软件滤波处理）。

- 对信号产生的电路，TIA 电路，使用 法拉第电磁笼屏蔽，对这块电路上一个 金属壳，金属壳连接 TIA 电路的 地！可以显著减小其输出的噪声。

  实测，对于 uA/nA 级别的测量，加入与不加入电磁笼不明显（即这个级别的测量，看电路设计性能，按需来看怎么优化），对于 pA 敏感的 TIA，加金属壳电磁笼会显著减小 TIA 输出噪声！所以对于 pA 级别的 TIA，必加。
  
  还可以多层电磁笼屏蔽，TIA 电路和 ADC 电路 （对模拟电路部分）加上（注意这部分电路紧凑、共地，且注意防止共地干扰，各自独立接地 到电源进来的的地），然后对整个电路板（数模电路）再加一层金属壳屏蔽电磁干扰，同样这个金属壳要接地（整个电路板的电源的地）。



软件上：ADC 内部的滤波环节配置；软件算法滤波。

- SD ARC 一般有 OSR 超采样 可以配置（32/64/256/512/1024等），ADC 模块的 输入调制频率 fM，采样频率 fS，一般有 fS = fM / OSR，且滤波器的第一个陷波在 这个 fS 频率处，可以根据实际需要进行配置。

- 平均法、滑动窗口，FIR / IIR 数字滤波 等（[如何把50Hz工频干扰定点清除-电子发烧友网 (elecfans.com)](https://www.elecfans.com/d/2096500.html)，[心电滤波(STM32)小结-CSDN博客](https://blog.csdn.net/2202_75407991/article/details/140814106)）。

  可做 FIR 滤波，50Hz工频滤除，测量频率 以上的噪声滤除。
...
- 电流输入端先进入运放 IN-；跨阻 Rf 应靠近运放的 IN-；Vout 从 Rf 引脚出。加入保护环。具体可以看 ADA4530 和 LMP7721 等芯片的有关手册里面的 推荐 PCB 图。

- 输入线路尽量短，减少漏电流。走线可以细一点，减小串扰。
...
- 在仿真软件里面跑一跑，可 TINA-TI 或 LTspice，放上实际选用的芯片型号，这些仿真会、也应该手动加上 要考虑的 运放内（运放输入脚电容）、外部（如信号源等效电容、线路杂散电容 等） 的 输入电容 等。

  看一看 各种性能指标，伯德图，相位裕量，噪声功率谱图，噪声功率密度 等。



- 工程上，要考虑实际 电路上各个地方 电阻 选型 的 精度。考虑每个电阻都是有个误差的（根据新型的精度是 1%、0.1% 等的），计算最差的情况，是否还能满足电路需求，让电路正确工作。



- 关于 TIA 部分电路的开窗。经过 ADA4530 的 手册 43 页 的 介绍 和 与大模型 battle，得出以下结论。
  关于开窗，首先是取消输入线路与保护环等线路之间通过阻焊层如绿油的漏电流；然后：

  绝缘体表面（包括阻焊层、PCB基材）在受到机械应力（压电效应）或摩擦（摩擦电效应）时，会产生静电荷积累。这些电荷若未被保护环有效抑制，会通过漏电路径流向高阻抗导体，导致测量误差。暴露的绝缘体（包括阻焊层下的基材）本身就是电荷积累的源头，因此必须通过开窗（去除阻焊）减少表面电荷的附着介质，并通过保护环直接驱动该区域，抑制电荷迁移。

  增大高阻抗线路与保护环的间距会因暴露绝缘面积增加导致表面电荷积累风险上升，反而降低电路性能。设计时需严格遵循推荐间距（15 mil）

  结论是 应该把 guard ring 内部所有电路都开窗（如果电路的测量级别为 pA / fA 乃至更小量级的话，就需要，所以按需来设计）。guard ring 贴合 被保护电路 即可，不用太远也不用过近。
...
- > 应使用 ESD 安全手套或手指护套来处理电路板，以免污染护肤油或其他调味品。（机翻勿扰）
  >
  > 避免让 电路板吸入空气，因为其中的含盐水分会严重降低性能。电路板需要存放在密封的低湿度容器中，并带有干燥剂包。
...
- > 在进行低电流噪声测量时，泄漏路径需要考虑干扰源。由于电流电平极低，干扰信号的任何微小拾取都可 能引入较大的误差。
  >
  > 通常，需要使用屏蔽外壳来更大限度地减少噪声拾取。
...
- 示波器进行噪声测量前，若有自校准功能，先自校准，然后，测量其本地噪声：

  - 将输入短接（输入端和底线的短接避免不要形成环形 而 减小受到 外部电磁干扰 的影响）；
  - 示波器设置采集为 20MHz 带宽限制（若测量低频信号，应开启这个，没必要拾取高频段信号）；
  - 并开启 AC 耦合模式来测量 而 不需要拾取直流分量（若是针对 如 0.1Hz~10Hz 的1/f 闪烁噪声 等 低频 信号的测量，则还是需要 使用直流 DC 耦合，因为 AC 耦合会有一定低频截止频率的滤波）。

- 要注意 测量仪器 的输入电阻，对于 高输出电阻 的 电路，若 测量 电压 的仪器的输入电阻不是远大于 被测电路的输出电阻，则会影响被测电路，使得测量示值偏低 而 不准确。
